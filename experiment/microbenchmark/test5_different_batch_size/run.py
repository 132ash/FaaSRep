...
def worker_task(client_id, workflow, parameters_all_round, result_queue, batch_size):
    """子进程执行的任务。"""
    local_results = []
    ...
    for i in range(ROUND):
        ...
        if i % (ROUND // 10) == 0 or i == ROUND - 1:
            print(f"Client {client_id}: Round {i+1} completed, batch sent", flush=True)
...
def write_result_to_file(system_mode, batch_size, median_latency, p99_latency, avg_throughput):
    ...
    if not result_file.exists():
        with open(result_file, 'w') as f:
            f.write("batch_size,median_latency,p99_latency,avg_throughput\n")
        print(f"📝 创建汇总结果文件: {result_file}", flush=True)
    
    # 追加结果数据
    with open(result_file, 'a') as f:
        f.write(f"{batch_size},{median_latency:.4f},{p99_latency:.4f},{avg_throughput:.4f}\n")
    
    print(f"📊 汇总结果已写入文件: {result_file}", flush=True)
    print(f"📊 数据: batch_size={batch_size}, median_latency={median_latency:.4f}, p99_latency={p99_latency:.4f}, avg_throughput={avg_throughput:.4f}", flush=True)

def analyze_all(workflow_name, system_mode, client_cnt, batch_size):
    print(f"🚀 开始测试 - 工作流: {workflow_name}, 模式: {system_mode}, 客户端: {client_cnt}, 批处理大小: {batch_size}", flush=True)
    ...
    for i in range(client_cnt):
        process = multiprocessing.Process(
            target=worker_task, 
            args=(i, workflow_name, parameters_all[i], result_queue, batch_size)
        )
    ...
    # --- 保存原始结果 ---
    # 文件名使用 batch_size 进行标识
    raw_result_filename = raw_results_dir / f"{workflow_name}_{batch_size}_raw.csv"
    ...
    avg_throughput = (client_cnt * ROUND) / total_time  # 转换为 RPS (请求数 / 总时间)
    
    print(f"", flush=True)
    print(f"📊 {workflow_name} 测试结果 (批处理大小: {batch_size}):", flush=True)
    ...
    # 直接写入结果文件
    write_result_to_file(system_mode, batch_size, median_e2e_latency, p99_e2e_latency, avg_throughput)
    
    print(f"✅ {workflow_name} 测试完成 (客户端: {client_cnt}, 批处理大小: {batch_size})", flush=True)
    sys.stdout.flush() 
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 5:
        print("用法: python run.py <workflow_name> <system_mode> <client_count> <batch_size>", flush=True)
        sys.exit(1)
    ...
    try:
        analyze_all(workflow_name, system_mode, client_cnt, batch_size)
    except Exception as e:

...
for SEGMENT_INDEX in "${TARGET_SEGMENT_INDICES[@]}"; do
    ...
    MANIFEST_FILE="$MANIFEST_ROOT/$TRACE/c4_zipf${ZIPF_PARAM}_segment_${SEGMENT_INDEX}.jsonl"
    ...
    if [ ! -f "$MANIFEST_FILE" ]; then
        ...
        echo "prepare it first with: bash $SCRIPT_DIR/prepare_boki_manifests.sh" >&2
        exit 1
    fi
    echo "Running Boki-SN trace=$TRACE segment=$SEGMENT_INDEX"
    SYSTEM_MODE=BOKI_SN python3 "$SCRIPT_DIR/run_boki_segment.py" \
        --segment "$SEGMENT_FILE" --manifest "$MANIFEST_FILE" --trace "$TRACE" \
        --workflow "$WORKFLOW" --seed "$SEED" --request-timeout "$REQUEST_TIMEOUT"
done

...
SUMMARY_FIELDS = [
    'trace', 'segment_index', 'configured_abort_prob',
    'actual_abort_count', 'success_count', 'success_p50', 'success_p99',
    'success_throughput',
]
...
def summarize_raw_file(raw_path, summary_path):
    ...
    if not measured:
        raise ValueError(f'no requests fall inside the core interval: {raw_path}')
    failures = [row for row in measured if row['status'] != 'ok']
    if failures:
        raise ValueError(
            f'{len(failures)} core requests failed; summary was not written')
    ...
    summary_row = {
        ...
        'configured_abort_prob': measured[0]['configured_abort_prob'],

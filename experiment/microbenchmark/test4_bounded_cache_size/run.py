...
def write_result_to_file(system_mode, cache_bound, median_latency, p99_latency, avg_throughput):
    """将汇总结果追加到最终结果文件"""
    # 每个 mode 对应一个文件夹
    ...
    if not result_file.exists():
        with open(result_file, 'w') as f:
            f.write("cache_bound,median_latency,p99_latency,avg_throughput\n")
        print(f"📝 创建汇总结果文件: {result_file}", flush=True)
    
    # 追加结果数据
    with open(result_file, 'a') as f:
        f.write(f"{cache_bound:.2f},{median_latency:.4f},{p99_latency:.4f},{avg_throughput:.4f}\n")
    
    print(f"📊 汇总结果已写入文件: {result_file}", flush=True)
    print(f"📊 数据: cache_bound={cache_bound:.2f}, median_latency={median_latency:.4f}, p99_latency={p99_latency:.4f}, avg_throughput={avg_throughput:.4f}", flush=True)

def analyze_all(workflow_name, system_mode, client_cnt, zipf_param, cache_bound):
    print(f"🚀 开始测试 - 工作流: {workflow_name}, 模式: {system_mode}, 客户端: {client_cnt}, zipf: {zipf_param:.2f}, 缓存比例: {cache_bound:.2f}", flush=True)
    ...
    # --- 保存原始结果 ---
    # 文件名使用 cache_bound 进行标识
    raw_result_filename = raw_results_dir / f"{workflow_name}_{cache_bound:.2f}_raw.csv"
    ...
    print(f"", flush=True)
    print(f"📊 {workflow_name} 测试结果 (缓存比例: {cache_bound:.2f}):", flush=True)
    ...
    # 直接写入结果文件
    write_result_to_file(system_mode, cache_bound, median_e2e_latency, p99_e2e_latency, avg_throughput)
    
    print(f"✅ {workflow_name} 测试完成 (客户端: {client_cnt}, 缓存比例: {cache_bound:.2f})", flush=True)
    sys.stdout.flush() 
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 6:
        print("用法: python run.py <workflow_name> <system_mode> <client_count> <zipf_param> <cache_bound>", flush=True)
        sys.exit(1)
    ...
    cache_bound = float(sys.argv[5])
    
    try:
        analyze_all(workflow_name, system_mode, client_cnt, zipf_param, cache_bound)
    except Exception as e:

...
WORKER_NODES = ['10.2.30.50', '10.2.30.62']
AGENT_PORT = 5001
...
def start_metrics_collection():
    ...
    for worker_ip in WORKER_NODES:
        try:
            response = requests.post(f'http://{worker_ip}:{AGENT_PORT}/start_metrics')
            if response.status_code == 200:
                print("成功启动数据收集进程。")
            else:
                print(f"启动数据收集进程失败: {response.text}")
        except requests.exceptions.RequestException as e:
            print(f"请求监控代理时发生错误: {e}", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    analyze_all_workflows("OPTIMISTIC")

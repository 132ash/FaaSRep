...
class Function:
    def __init__(self, host_addr, function_pos, client, function_info:FunctionInfo, port_controller, default_container_num, input, output):
        ...
        if function_pos[self.info.function_name] == host_addr:
            ...
            time.sleep(2) # wait for a while to make sure all containers are ready
        
    # put the request into request queue
    def send_request(self, create_timestamp, transaction_id, write_set, term):
        data = {'transaction_id': transaction_id, 'write_set':write_set, 'create_timestamp': create_timestamp, 'term':term}

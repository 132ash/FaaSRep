...
class FunctionManager:
    ...
    def run(self, create_timestamp, function_name, transaction_id, write_set, term):
        # print('run', function_name, request_id, runtime, input, output, to, keys)
        if function_name not in self.functions:
            raise Exception("No such function!")
        return self.functions[function_name].send_request(create_timestamp, transaction_id, write_set, term)

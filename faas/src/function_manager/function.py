...
class Function:
    def __init__(self, client, function_info:FunctionInfo, port_controller, node_list, default_container_num, input, output,function_pos):
        self.client = client
    ...
    def send_request(self, transaction_id, write_set, snapshot_interval):
        data = {'transaction_id': transaction_id,
                 'write_set':write_set, 'snapshot_interval':snapshot_interval}
    ...
    def init_container(self, container: Container):
        container.init(self.info.workflow_name, self.info.function_name,  self.VALIDATOR_ADDR, self.node_list, self.input, self.output, self.function_pos, container.port)

...
class TransactionState:
    def __init__(self, transaction_id: str, all_func: List[str], write_set, snapshot_interval):
...
class WorkerSPManager:
    ...
    def get_state(self, retry_after_abort, transaction_id: str,  write_set, snapshot_interval) -> TransactionState:
        self.lock.acquire()
        ...
        if transaction_id not in self.states or retry_after_abort:
            self.states[transaction_id] = TransactionState(transaction_id, self.func, write_set, snapshot_interval)
        else:

...
class RepairInfo:
    ...
    def construct_repair_metadata(self, batch_id, expired_keys, crosstx_subjection, RYW_subjection, function_pos_per_tx, worker_set):
        
        for tx_id, tx_info in crosstx_subjection.items():
            for func, func_info in tx_info.items():
                if self.fast_path_enabled:
                    ip = function_pos_per_tx[tx_id][func]['ip']
                    tx_info_dict = self.get_info_dict(batch_id, ip, tx_id)
                    tx_info_dict[func] = func_info
        ...
        for tx_id, tx_info in expired_keys.items():
            for func, expired_keys_dict in tx_info.items():
                ip = function_pos_per_tx[tx_id][func]['ip']
                func_info_dict = self.get_info_dict(batch_id, ip, tx_id, func)
                expired_keys_per_ip[ip].union(set(expired_keys_dict.keys()))
    ...
    def get_info_dict(self, batch_id, ip, tx_id, func=""):
        ...
        return tx_dict.setdefault(func, {'dirty':False, 'key_subjection':{'up_cnt':0, 'upstream_keys':{}}}) if func else tx_dict

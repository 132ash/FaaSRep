...
class RepairEngine:
    ...
    def trigger_repair(self, batch_id, transaction_list, workflow_name_per_tx, function_pos_per_tx, expired_keys, dirty_set, downstream_func_table, upstream_func_table):
        self.repairing_batches[batch_id] = {'cond': event.Event(), 'finished': False, "repaired":False}
        self.repairing_batches[batch_id]['cond'].clear()
        self.repair_batch(batch_id, transaction_list, workflow_name_per_tx, function_pos_per_tx, expired_keys, dirty_set, downstream_func_table, upstream_func_table)
    ...
    def trigger_function(self, workflow_name, transaction_id, function_name, ip, expired_keys, dirty_set, downstream_func_table, upstream_func_table, batch_id):
    ...
    def repair_batch(self,batch_id, transaction_list, workflow_name_per_tx, function_pos_per_tx, expired_keys, dirty_set, downstream_func_table, upstream_func_table):
        # allocate works
        start = time.time()
        jobs = []
        for tx_id in transaction_list:
            workflow_name = workflow_name_per_tx[tx_id]
            logging.info(f"repairing {workflow_name}, ID:{tx_id} with expired set {expired_keys[tx_id]}" )
            ...
            for n in start_functions:
                ...
                jobs.append(gevent.spawn(self.trigger_function, workflow_name, tx_id, n, ip, expired_keys[tx_id], dirty_set[tx_id], downstream_func_table[tx_id], upstream_func_table[tx_id], batch_id))
        gevent.joinall(jobs)   
        end = time.time()
        return end - start
